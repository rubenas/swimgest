...
function forgottenPassEmail($swimmer)
{
    ...
    $body .= '<p>Para cambiar tu contraseña en la web de '.$_SERVER['HTTP_HOST'].', pincha 
                <a href="' . (isset($_SERVER['HTTPS']) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . '/swimgest/index.php?controller=login&action=forgottenPass&id=' . $swimmer->getId() . '&token=' . $swimmer->getResetPassToken() . '">AQUÍ</a> 
    ...
    $body .= '<p>Equipo web SwimGest.</p>';

    $body .= '<p><img src="' . (isset($_SERVER['HTTPS']) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'] . '/swimgest/public/img/default-logo.svg" style="width:150px"></p>';
